...
class FilmeRepository:
    ...
    def add_filme(self, filme):
        db = get_db()
        cursor = db.cursor()
        cursor.execute("INSERT INTO filme (titulo, genero, ano_lancamento) VALUES (?, ?, ?)",
    ...
    def update_filme(self, filme):
        db = get_db()
        cursor = db.cursor()
        cursor.execute("UPDATE filme SET titulo = ?, genero = ?, ano_lancamento = ? WHERE id = ?",
    ...
    def delete_filme(self, id):
        db = get_db()
        cursor = db.cursor()
        cursor.execute("DELETE FROM filme WHERE id = ?", (id,))
        db.commit()
